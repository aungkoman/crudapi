...
include('../../../model/v1/USER.php'); // user Model
$user = new USER(); // create new user object
$method = $_SERVER['REQUEST_METHOD']; // get request method
//return_fail('hello fail',"hello");
// middleware_super_user($_POST); // middleware usage 
switch ($method) {
	case 'POST':
        $request_data = $_POST;
        $ops_type = (string) isset($request_data['ops_type']) ? sanitize_str($request_data['ops_type'],"user->ops_type") :  return_fail('user->ops_type : ops_type is not defined in requested data'); // ops_type sanitize string
        //print_r($request_data);
        switch ($ops_type){
            case 'insert':
                //middleware_admin($request_data);
                $user->insert($request_data);
                break;
            case 'login':
            ...
            case 'delete':
                ...
                break;
            default :
                return_fail('user : unknow_ops_type',$ops_type);
                break;
        }
        break;
	default:
		...
		return_fail("user : unknow_method",$method);
		break;
}
?>