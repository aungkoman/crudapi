...
include('../../../model/v1/MULTICONTENT.php'); // multicontent Model
$multicontent = new MULTICONTENT(); // create new multicontent object
$method = $_SERVER['REQUEST_METHOD']; // get request method
// middleware_super_user($_POST); // middleware usage 
switch ($method) {
	case 'POST':
        $request_data = $_POST;
        $ops_type = (string) isset($request_data['ops_type']) ? sanitize_str($request_data['ops_type'],"multicontent->ops_type") :  return_fail('multicontent->ops_type : ops_type is not defined in requested data'); // ops_type sanitize string
        switch ($ops_type){
            case 'insert':
                //middleware_admin($request_data);
                $multicontent->insert($request_data);
                break;
            case 'select':
                //middleware_user($request_data);
                $multicontent->select($request_data);
                break;
            case 'update':
                //middleware_admin($request_data);
                $multicontent->update($request_data);
                break;
            case 'delete':
                //middleware_admin($request_data);
                $multicontent->delete($request_data);
                break;
            default :
                return_fail('multicontent : unknow_ops_type',$ops_type);
                break;
        }
        break;
	default:
		...
		return_fail("multicontent : unknow_method",$method);
		break;
}
?>