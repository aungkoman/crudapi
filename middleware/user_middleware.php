...
function middleware_login($data){
	...
	//print_r($data);
	//  1. name 
	$username = (string) isset($data['username']) ? sanitize_str($data['username'],"user_middleware->login : username") :  return_fail('user_middleware->login : username is not defined in requested data');
	//  1. password 
	$password = (string) isset($data['password']) ? sanitize_str($data['password'],"user_middleware->login : password") :  return_fail('user_middleware->login : password is not defined in requested data');
	$users = R::find('user', ' username = ?', [ $username ] );
	if(count($users) == 0 ) return_fail("user_middleware->login : username is not found in our system",$username);
	$return_data = array();
    foreach($users AS $index=>$user){
        $return_data[] = $user;
	}
	...
	if(password_verify($password,$user->password)){
		...
		*/
		try{
			...
			retunr_fail("user_middleware->login : JWT Encode Exception",$exp->getMessage());
		}
	}else{
